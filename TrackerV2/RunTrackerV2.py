...
def vector_to_quaternion(vec):
    """
    Input:
        vec : iterable (X, Y, Z) in AIRCRAFT frame
              (X forward, Y left, Z up)

    Behavior:
        Constrains roll = 0; builds quaternion from yaw & pitch only.
    ...
    yaw   = np.arctan2(Ya, Xa)                # ψ about +Z (left-turn positive)
    pitch = np.arctan2(Za, np.hypot(Xa, Ya))  # θ about +Y (nose-up positive)
    roll  = 0.0                               # φ about +X (constrained)
    ...
    return (qx, qy, qz, qw, yaw_deg, pitch_deg, roll_deg)
...
def QueeryTracker(tracklets_q, start_time, state: TrackerState):
    ...
    out = {
        ...
        "OmegaX": float('nan'), "OmegaY": float('nan'), "OmegaZ": float('nan'),
        "Yaw": float('nan'), "Pitch": float('nan'), "Roll": float('nan'),
    ...
    t = track.tracklets[0]
    y =  -float(t.spatialCoordinates.x) + 0     #repalce with actual shift later
    z =  float(t.spatialCoordinates.y) + 50     #repalce with actual shift later 
    x =  float(t.spatialCoordinates.z) + 170    #repalce with actual shift later   

    try:
        qx, qy, qz, qw, yaw_deg, pitch_deg, roll_deg = vector_to_quaternion([x, y, z])
    except ValueError:
    ...
    out.update({
        ...
        "OmegaX": float(omega[0]), "OmegaY": float(omega[1]), "OmegaZ": float(omega[2]),
